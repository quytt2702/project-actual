...
@section('master.title', 'Chỉnh sửa sản phẩm')
...
<div class="row m-t-30">
  <div class="col-sm-9">
    <div class="white-box">
      <h3 class="box-title m-b-0">Chỉnh sửa sản phẩm</h3>
      <br>
      <div class="form-horizontal">
        <div class="form-group">
          <label class="col-md-12">Mã sản phẩm</label>
          <div class="col-md-12">
            <input id="ma_san_pham" class="form-control" value="{{ $sanPham->san_pham_ma }}" placeholder="Nhập mã sản phẩm tại đây">
          </div>
        </div>
        <div class="form-group">
          <label class="col-md-12">Tên sản phẩm</label>
          <div class="col-md-12">
            <input id="ten_san_pham" class="form-control" value="{{ $sanPham->san_pham_ten }}" placeholder="Nhập tên sản phẩm tại đây">
          </div>
        </div>
        <div class="form-group">
          ...
          <div class="col-md-12">
            <div class="input-group">
              <input id="link" class="form-control" placeholder="Dữ liệu được sinh từ tiêu đề" value="{{ $sanPham->san_pham_url }}" disabled>
        ...
        <div class="form-group">
          ...
          <div class="col-md-4">
            <input id="gia_ban_khuyen_mai" class="form-control" type="number" value="{{ $sanPham->san_pham_gia_ban_ao }}" value="" placeholder="Giá bán khuyến mãi" style="text-decoration: line-through;">
          </div>
          <div class="col-md-8">
        ...
        <div class="form-group">
          ...
          <div class="col-md-4">
            <input id="gia_ban_thuc_te" class="form-control" value="{{ $sanPham->san_pham_gia_ban_thuc_te }}" type="number" placeholder="Giá bán thực tế">
          </div>
          <div class="col-md-8">
        ...
        <div class="form-group">
          ...
          <div class="col-md-12">
            <textarea id="mo_ta_ngan" name="editor" class="form-control editor" rows="5" placeholder="Nhập nội dung mô tả ngắn tại đây">{{ $sanPham->san_pham_mo_ta }}</textarea>
          </div>
        </div>
          ...
          @foreach($tabs as $item)
            @if($item->is_open==='YES')
              <div role="tabpanel" class="tab-pane fade {{ ($index === 1) ? 'active in':'' }}" id="tab{{$item->id}}">
                <div class="col-md-12">
                  <div class="form-group">
                      <textarea id="san_pham_noi_dung_tab_{{$index}}" {{ ($item->is_open === 'YES') ? '':'disabled' }} class="form-control" rows="5" placeholder="Nhập nội dung chính của sản phẩm">{!! $noiDung[$item->id -1] !!}</textarea>
                      <script>
        ...
        <div class="form-group">
          ...
          <div class="col-md-12">
            <input id="keyword" type="text" placeholder="Enter để thêm từ khoá" value="{{ $sanPham->san_pham_keyword }}" class="form-control" data-role="tagsinput">
          </div>
        </div>

        <div class="form-actions text-right">
          <button id="btnSubmit" hash="{{ $sanPham->id }}" class="btn btn-info right"> <i class="fa fa-check"></i> Sửa sản phẩm</button>
  ...
  <div class="col-sm-3">
    <div class="panel panel-default">
      <div class="panel-heading" role="tab" id="headingTwo">
        ...
        @php
          $index = 1;
          $chuyenMuc = json_decode($sanPham->san_pham_id_chuyen_muc, true);
        @endphp
        @foreach($chuyenMucSanPham as $item)
          <div class="checkbox checkbox-info">
            <input type="checkbox" name="chuyenMucSanPham" class="checkbox" id="chk-ignore-case-{{ $index }}" value="{{ $item->id }}" {{ in_array($item->id ,$chuyenMuc) ? 'checked':'' }}>
    ...
    <div class="panel panel-default">
      ...
      <div id="headingThree" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingThree" aria-expanded="true" style="">
        <div class="panel-body" style="padding: 15px;">
          <img id="ckfinder-input-2" src="{{ $sanPham->san_pham_hinh_dai_dien }}" style="width: 100%" class="m-b-10">
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body p-t-10">
        <div id="collapseTwo" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingTwo" aria-expanded="true" style="">
            <input id="tags" type="text" placeholder="Enter để thêm vào tags" value="{{ $sanPham->san_pham_tags }}" class="form-control" data-role="tagsinput">
        </div>
      </div>
    </div>
  </div>
